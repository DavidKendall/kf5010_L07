...
static sem_t count1Done;
static sem_t count2Done;
...
int main (void) {
    ...
    console_init();
    srand(time(NULL));
    rc = sem_init(&count1Done, 0, 0);
    assert(rc == 0);
    rc = sem_init(&count2Done, 0, 1);
    assert(rc == 0);
